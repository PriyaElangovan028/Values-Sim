...
function PortalPillars({ onSelectScene }) {
  ...
  return (
    <>
      ...
      <group ref={iconRef}>
        {positions.map((pos, i) => (
          <mesh key={i} position={[pos[0], 7, pos[2] + 0.1]}>
            <planeGeometry args={[1.2, 1.2]} />
            <meshBasicMaterial map={iconTextures[i]} transparent />
          </mesh>
        ))}
      </group>
    </>
...
export default function Scene() {
  ...
  const scenes = {
    leadingChange: (
      <LeadingChangeScene goBack={() => setCurrentScene("portalHub")} />
    ),
  };
  return (
    <div className="scene-wrapper" style={{ width: "100vw", height: "100vh", position: "relative" }}>
      {currentScene === "portalHub" && (
        <Canvas
          style={{ width: "100%", height: "100%" }}
          camera={{ position: [0, 10, 25], fov: 50, near: 1, far: 100 }}
          onCreated={({ scene }) => (scene.background = null)}
        >
          <ambientLight intensity={0.8} />
          <directionalLight position={[10, 15, 10]} intensity={1.5} />

          <Environment files="/textures/sky.jpg" background />
      ...
      {currentScene === "leadingChange" && (
        <div
          style={{
            width: "100%",
            height: "100%",
            backgroundImage: "url('/textures/leading_change_bg.jpg')",
